...
class ARISEProcessor:
    ...
    def create_datacube(self) -> xr.Dataset:
        ...
        # Initialize dictionary to hold processed data
        self.processed_data = {}

        # Parallel processing of variables
        self.logger.info('!!!' * 80)
        self.logger.info("Processing variables in parallel...")
        self.logger.info(f"Num CPUs: {os.cpu_count() or 4}")
        self.logger.info('!!!' * 80)

        with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as executor:
            # Submit all variables to the executor
            future_to_var = {executor.submit(self.process_variable, var, files_by_var[var]): var for var in self.variables}
            ...
            for future in concurrent.futures.as_completed(future_to_var):
                ...
                try:w
                    ...
                    self.logger.error(f"Error processing variable {var}: {e}")
...
if __name__ == "__main__":
    ...
    final_datacube = processor.create_datacube()
